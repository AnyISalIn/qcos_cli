...
def get_parser():
    ...
    upload_file_parser = sub.add_parser('upload_file', help='upload local file')
    upload_file_parser.add_argument('local_file', help='local file path')
    upload_file_parser.add_argument('remote_file', help='remote file path, start with /, end not /')
    upload_file_parser.add_argument('--overwrite', action='store_true', default=False, help='overwrite remote file')
    upload_file_parser.set_defaults(func=upload_file)

    stat_file_parser = sub.add_parser('stat_file', help='show remote file status')
    stat_file_parser.add_argument('remote_file', help='remote file path, start with /, end not /')
    stat_file_parser.set_defaults(func=stat_file)

    update_file_parser = sub.add_parser('update_file', help='update file')
    update_file_parser.add_argument('remote_file', help='remote file path, start with /, end not /')
    update_file_parser.add_argument('--biz_attr', default=None, help='update file attr')
    ...
    create_folder_parser = sub.add_parser('create_folder', help='create folder')
    create_folder_parser.add_argument('remote_folder', help='remote folder path, start with /, end must /')
    create_folder_parser.set_defaults(func=create_folder)

    update_folder_parser = sub.add_parser('update_folder', help='update folder')
    update_folder_parser.add_argument('remote_folder', help='remote folder path, start with /, end must /')
    update_folder_parser.add_argument('--biz_attr', default=None, help='update folder attr')
    update_folder_parser.set_defaults(func=update_folder)

    stat_folder_parser = sub.add_parser('stat_folder', help='show remote folder status')
    stat_folder_parser.add_argument('remote_folder', help='remote folder path, start with /, end must /')
    stat_folder_parser.set_defaults(func=stat_folder)

    del_folder_parser = sub.add_parser('del_folder', help='del folder')
    del_folder_parser.add_argument('remote_folder', help='del folder path, start with /, end must /')
    del_folder_parser.set_defaults(func=del_folder)

    list_folder_parser = sub.add_parser('list_folder', help='list folder')
    list_folder_parser.add_argument('remote_folder', help='remote folder path, start with /, end must /')
